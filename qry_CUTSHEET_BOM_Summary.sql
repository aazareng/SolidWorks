/* ============================================================================
   ...
   COLUMNS PRODUCED
     ...
     MFG_ZERO_QTY             - mfg-dept parts where RawCost = 0 (need est.)
   ============================================================================ */

SELECT

    /* STOCK (PURCH): Department = 'STOCK', StockedPartNumber NOT starting with '8' or '9' */
    ...
    /* MECH COST: non-purchased (not STOCK / SPECIAL ORDER), non-assembly (part# not 'A*') */
    ...
    /* STOCK (MFG): Department = 'STOCK', StockedPartNumber starts with '8' or '9' */
    SUM(IIF(b.Department = 'STOCK'
    ...
    /* TOTAL QTY: all non-assembly rows */
    SUM(IIF(Left(Nz(b.PART_NUMBER, ''), 1) <> 'A',
        Nz(b.QTY, 0), 0))                                     AS TOTAL_QTY,

    /* MFG_ZERO_QTY: mfg-dept rows where RawCost = 0, used to estimate missing costs */
...
SELECT
    ...
    /* BOM_MATERIAL_COST (partial - excludes ELE_COST from bom_ELE)
       Full MATERIAL_COST = BOM_MATERIAL_COST + ELE_COST */
    (s.STOCK_PURCH_COST + s.SP_COST)
    + s.MECH_COST
    + (s.LATHE_COST + s.MILL_COST + s.PUNCH_COST + s.ROUTER_COST + s.STOCK_MFG_COST)
    + IIF(
